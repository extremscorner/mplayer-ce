...
cache_vars_t* cache_init(int size,int sector){
  int num;
#if !defined(WIN32) && !defined(__OS2__) && !defined ( GEKKO )
  ...
  return s;
	...
	static unsigned char wiiThread_Stack[STACKSIZE];
...
#elif defined(GEKKO)
	thread_kill=1;
	usec_sleep(FILL_USLEEP_TIME); //scip: just to make sure
...
/*
#if defined(GEKKO)
static void *wiiThreadProc(void *param){
	//signal(SIGTERM,exit_sighandler); // kill
  while(!thread_kill){
    if(!cache_fill((cache_vars_t*)param)) usec_sleep(FILL_USLEEP_TIME); // idle
	 	cache_execute_control((cache_vars_t*)param);
...
#elif defined(GEKKO)
	thread_kill=0;
	//extern int LWP_CreateThread(void *thethread,void* (*entry)(void *),void *arg,void *stackbase,unsigned int stack_size, unsigned char prio);
	LWP_CreateThread(&stream->cache_pid, ThreadProc, s, wiiThread_Stack, STACKSIZE, 80);
#else   // OS2
