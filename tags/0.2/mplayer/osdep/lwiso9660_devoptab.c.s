...
int _DVD_fstat_r (struct _reent *r, int fd, struct stat *st)
{
...
#endif
int _DVD_seek_r (struct _reent *r, int fd, int pos, int dir)
{
		struct FILESTATE *filestate=(struct FILESTATE *)fd;
	int newpos=0;
...
int _DVD_read_r (struct _reent *r, int fd, char *ptr, int len)
{
	struct FILESTATE *filestate=(struct FILESTATE *)fd;
...
//				printf("Root Length: %d\n",rootdirlength);
	...
	if(fstype>=0)
	{
		...
		DVD_ScanContent_recurse(fstype==1 ? 1 : 0, direntryptr,0,rootsect,rootdirlength,&fake_inodecounter);
//		debug_dump_tree();
	}	
	free(sectbuf);
	return 0;
