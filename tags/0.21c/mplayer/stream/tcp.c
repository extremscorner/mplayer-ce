...
#else
	int retc=0;
	if( (retc=connect( socket_server_fd, (struct sockaddr*)&server_address, server_address_size ))==-1 ) {
#endif
...
#if !defined(GEKKO) // select not yet implemented in GEKKO
...
#if !defined(GEKKO)  //getsockopt not yet implemented in GEKKO
#ifndef HAVE_WINSOCK2
	fcntl( socket_server_fd, F_SETFL, fcntl(socket_server_fd, F_GETFL) & ~O_NONBLOCK );
#else
	val = 0;
