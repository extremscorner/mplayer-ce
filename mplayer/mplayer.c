...
// check .sub
  ...
  //char tmp_fname_noext[256];
  //strcpy_strip_ext2(tmp_fname_noext, filename);
  //char * sub_exts[] = {  "sub", "srt", "txt" };
  //strcat(tmp_fname_noext,".srt");
//dvd trazi velika slova SRT
  //printf("\t\n%s\n",tmp_fname_noext); //sleep(2);
  
  //add_subtitles (tmp_fname_noext, mpctx->sh_video->fps, 1);
  
  //add_subtitles ("smb:/test/test.srt", mpctx->sh_video->fps, 1);
  //add_subtitles ("fat3:/test.srt", mpctx->sh_video->fps, 1);
