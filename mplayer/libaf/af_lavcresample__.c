...
{    
  ...
  //alloc tmp samples instead of using the stack
  //(for platforms that have a small stack size, eg. Wii)
  if(s->tmps_alloc < out_len)
  {
      s->tmps_alloc = out_len;
      for(i=0; i<chans; i++)
          s->tmps[i] = realloc(s->tmps[i], out_len*sizeof(int16_t));
  }  
...
//      ret= av_resample(s->avrctx, tmp[i], s->in[i], &consumed, in_len, out_len, i+1 == chans);
      ret= av_resample(s->avrctx, s->tmps[i], s->in[i], &consumed, in_len, out_len, i+1 == chans);
  }
  out_len= ret;
  ...
  if(chans==1){
     ...
     memcpy(out, s->tmps[0], out_len*sizeof(int16_t));
  }else if(chans==2){
      for(j=0; j<out_len; j++){
          //*(out++)= tmp[0][j];
          //*(out++)= tmp[1][j];
